...
import com.hangeulbot.common.HangeulbotConsonants;
import com.hangeulbot.dao.APIDao;
import com.hangeulbot.utils.FileUtil;
import com.hangeulbot.utils.HangeulDisassembler;
...
import static com.hangeulbot.common.HangeulbotConsonants.apiUrl;
...
public class APIServiceImpl implements APIService {
    ...
    public HashMap<String, Object> loginByUserId(String userId) {
        ...
        log.debug(hangeulbotUser.toString());
        //아이 있음
        if (hangeulbotUser.getHangeulbotChildren() != null&&hangeulbotUser.getHangeulbotChildren().size()!=0) {
            resultMap.put("childRegistered", true);
            resultMap.put("token", Jwts.builder().setSubject(hangeulbotUser.getUserId()).claim("roles", "parent").
                    setIssuedAt(new Date()).signWith(SignatureAlgorithm.HS256, "secretkey").compact());

            //todo 아이 프로필 사진 경로 맞춰주자!! (Refac)
            List<HangeulbotChild> hangeulbotChildren = hangeulbotUser.getHangeulbotChildren();
    ...
    private void insertPhoenixInfo
        ...
        if(hangeulbotWord.getWordKorean().length()==hangeulbotUserWordLog.getInsertedAnswer().length()){
            ...
            for(int i=0;i<answerList.size();i++){
                ...
                //초성 기록을 가져온다.
                HangeulbotChoPhoenix hangeulbotChoPhoenix =
                        apiDao.getChoPhoenixByChoSungAndHangeulbotChild(cho,hangeulbotchild);
                if(hangeulbotChoPhoenix!=null){
                    hangeulbotChoPhoenix.setTestCount(hangeulbotChoPhoenix.getRightCount()+1);
                    if(cho==insertedCho){
                        hangeulbotChoPhoenix.setRightCount(hangeulbotChoPhoenix.getRightCount()+1);
                    }
                }else{
                    hangeulbotChoPhoenix = new HangeulbotChoPhoenix();
                    hangeulbotChoPhoenix.setChoSung(cho);
                    hangeulbotChoPhoenix.setHangeulbotChild(hangeulbotchild);
                    hangeulbotChoPhoenix.setTestCount(1);
                    if(cho==insertedCho){
                        hangeulbotChoPhoenix.setRightCount(1);
                    }else{
                        hangeulbotChoPhoenix.setRightCount(0);
                    }
                }
                apiDao.saveChoPhoenix(hangeulbotChoPhoenix);

                //중성 기록을 가져온다.
                HangeulbotJungPhoenix hangeulbotJungPhoenix =
                        apiDao.getJungPhoenixByJungSungAndHangeulbotChild(jun,hangeulbotchild);
                if(hangeulbotJungPhoenix!=null){
                    hangeulbotJungPhoenix.setTestCount(hangeulbotJungPhoenix.getRightCount()+1);
                    if(jun==insertedJun){
                        hangeulbotJungPhoenix.setRightCount(hangeulbotJungPhoenix.getRightCount()+1);
                    }
                }else{
                    hangeulbotJungPhoenix = new HangeulbotJungPhoenix();
                    hangeulbotJungPhoenix.setJungSung(jun);
                    hangeulbotJungPhoenix.setHangeulbotChild(hangeulbotchild);
                    hangeulbotJungPhoenix.setTestCount(1);
                    if(cho==insertedCho){
                        hangeulbotJungPhoenix.setRightCount(1);
                    }else{
                        hangeulbotJungPhoenix.setRightCount(0);
                    }
                }
                apiDao.saveJungPhoenix(hangeulbotJungPhoenix);


                //종성기록을 가져온다.(종성이 공란이면 a)
                if(jon!='a'){
                    HangeulbotJongPhoenix hangeulbotJongPhoenix =
                        apiDao.getJongPhoenixByJongSungAndHangeulbotChild(jon,hangeulbotchild);
                    if(hangeulbotJongPhoenix!=null){
                        hangeulbotJongPhoenix.setTestCount(hangeulbotJungPhoenix.getRightCount()+1);
                        if(jon==insertedJon){
                            hangeulbotJongPhoenix.setRightCount(hangeulbotJungPhoenix.getRightCount()+1);
                        }
                    }else{
                        hangeulbotJongPhoenix = new HangeulbotJongPhoenix();
                        hangeulbotJongPhoenix.setJongSung(jon);
                        hangeulbotJongPhoenix.setHangeulbotChild(hangeulbotchild);
                        hangeulbotJongPhoenix.setTestCount(1);
                        if(cho==insertedCho){
                            hangeulbotJongPhoenix.setRightCount(1);
                        }else{
                            hangeulbotJongPhoenix.setRightCount(0);
                        }
                    }
                    apiDao.saveJongPhoenix(hangeulbotJongPhoenix);
                }else{
    ...
    public HashMap<String, Object> getChildStats(String childId,Pageable pageable) {
        ...
        //파닉스 숙련도 ( 초, 중, 종성 )
        ArrayList<HashMap<Character,String>> phoenixList = getChoPhoenixByChild(hangeulbotChild);
        ArrayList<HashMap<Character,String>> jungPhoenix = getJungPhoenixByChild(hangeulbotChild);
        ArrayList<HashMap<Character,String>> jongPhoenix = getJongPhoenixByChild(hangeulbotChild);

        //단어 숙련도 표시 todo : 기록없음 로그
        HashMap<String,String> wordAchievementMap = new HashMap<>();
        List<HangeulbotWordAchievement> wordAchievementList =
                apiDao.getWordAchievementInfoByHangeulbotChild(hangeulbotChild);
        for(int i=0;i<wordAchievementList.size();i++){
            HangeulbotWordAchievement tempAchievement = wordAchievementList.get(i);
            String tempWord = wordAchievementList.get(i).getHangeulbotWord().getWordKorean();
            ...
            if(tempAchievement.getTestCount()>10){
                if(tempWordAnswerRate>=70.0){
                    wordAchievementMap.put(tempWord,"a");
                }else if(tempWordAnswerRate<70.0&&tempWordAnswerRate>=50.0){
                    wordAchievementMap.put(tempWord,"b");
                }else{
                    wordAchievementMap.put(tempWord,"c");
                }
            }else{
        ...
        resultMap.put("wordAchievementMap",wordAchievementMap);
        //파닉스 정답율
        resultMap.put("phoenixList",phoenixList);
        resultMap.put("jungPhoenix",jungPhoenix);
        resultMap.put("jongPhoenix",jongPhoenix);
    ...
    private ArrayList<HashMap<Character,String>> getJongPhoenixByChild(HangeulbotChild hangeulbotChild) {
        ArrayList<HashMap<Character,String>> jongPhoenix = new ArrayList<>();
        for(int i=0;i<HangeulbotConsonants.JON.length;i++){
            HashMap<Character,String> tempHashMap = new HashMap<>();
            HangeulbotJongPhoenix hangeulbotJongPhoenix =
                    apiDao.getJongPhoenixByJongSungAndHangeulbotChild(HangeulbotConsonants.JON[i],hangeulbotChild);
            if(hangeulbotJongPhoenix!=null){
                float choAnswerRate = (hangeulbotJongPhoenix.getRightCount()/hangeulbotJongPhoenix.getTestCount())*100;
                if(hangeulbotJongPhoenix.getTestCount()>10){
                    if(choAnswerRate>=70.0){
                        tempHashMap.put(HangeulbotConsonants.JON[i],"a");
                    }else if(choAnswerRate<70.0&&choAnswerRate>=50.0){
                        tempHashMap.put(HangeulbotConsonants.JON[i],"b");
                    }else{
                        tempHashMap.put(HangeulbotConsonants.JON[i],"c");
                    }
                }else{
                    if(choAnswerRate>=80.0){
                        tempHashMap.put(HangeulbotConsonants.JON[i],"a");
                    }else if(choAnswerRate<80.0&&choAnswerRate>=60.0){
                        tempHashMap.put(HangeulbotConsonants.JON[i],"b");
                    }else{
                        tempHashMap.put(HangeulbotConsonants.JON[i],"c");
                    }
                ...
                tempHashMap.put(HangeulbotConsonants.JON[i],"d");
            }
            jongPhoenix.add(tempHashMap);
        }
        return jongPhoenix;
    }


    private ArrayList<HashMap<Character,String>> getJungPhoenixByChild(HangeulbotChild hangeulbotChild) {
        ArrayList<HashMap<Character,String>> jungPhoenix = new ArrayList<>();
        for(int i=0;i<HangeulbotConsonants.JUN.length;i++){
            HashMap<Character,String> tempHashMap = new HashMap<>();
            HangeulbotJungPhoenix hangeulbotJungPhoenix =
                    apiDao.getJungPhoenixByJungSungAndHangeulbotChild(HangeulbotConsonants.JUN[i],hangeulbotChild);
            if(hangeulbotJungPhoenix!=null){
                float choAnswerRate = (hangeulbotJungPhoenix.getRightCount()/hangeulbotJungPhoenix.getTestCount())*100;
                if(hangeulbotJungPhoenix.getTestCount()>10){
                    if(choAnswerRate>=70.0){
                        tempHashMap.put(HangeulbotConsonants.JUN[i],"a");
                    }else if(choAnswerRate<70.0&&choAnswerRate>=50.0){
                        tempHashMap.put(HangeulbotConsonants.JUN[i],"b");
                    }else{
                        tempHashMap.put(HangeulbotConsonants.JUN[i],"c");
                    }
                }else{
                    if(choAnswerRate>=80.0){
                        tempHashMap.put(HangeulbotConsonants.JUN[i],"a");
                ...
                tempHashMap.put(HangeulbotConsonants.JUN[i],"d");
            }
            jungPhoenix.add(tempHashMap);
        }
        return jungPhoenix;
    }

    private ArrayList<HashMap<Character,String>> getChoPhoenixByChild(HangeulbotChild hangeulbotChild) {
        ArrayList<HashMap<Character,String>> choPhoenix = new ArrayList<>();
        for(int i=0;i<HangeulbotConsonants.CHO.length;i++){
            HashMap<Character,String> tempHashMap = new HashMap<>();
            HangeulbotChoPhoenix hangeulbotChoPhoenix =
                    apiDao.getChoPhoenixByChoSungAndHangeulbotChild(HangeulbotConsonants.CHO[i],hangeulbotChild);
            if(hangeulbotChoPhoenix!=null){
                float choAnswerRate = (hangeulbotChoPhoenix.getRightCount()/hangeulbotChoPhoenix.getTestCount())*100;
                if(hangeulbotChoPhoenix.getTestCount()>10){
                    if(choAnswerRate>=70.0){
                        tempHashMap.put(HangeulbotConsonants.CHO[i],"a");
                    }else if(choAnswerRate<70.0&&choAnswerRate>=50.0){
                        tempHashMap.put(HangeulbotConsonants.CHO[i],"b");
                    }else{
                        tempHashMap.put(HangeulbotConsonants.CHO[i],"c");
                    }
                }else{
                    if(choAnswerRate>=80.0){
                        tempHashMap.put(HangeulbotConsonants.CHO[i],"a");
                ...
                tempHashMap.put(HangeulbotConsonants.CHO[i],"d");
            }
            choPhoenix.add(tempHashMap);
        }
        return choPhoenix;
    }
